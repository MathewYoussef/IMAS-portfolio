...
**Primary objective:** establish trimmed-mean concentration trajectories (DAD-led, Chrom corroborating) that serve as the canonical truth for judging whether reflectance-derived rankings preserve the same ordering; dose profiles are retained as contextual metadata rather than direct calibration targets.
